...
def bootstrap():
    """
    Automatically creates a virtual environment and install required dependencies.

    If the script is not running inside a venv, it creates one, installs packages,
    verifies mpremote, then re-executes itself inside the venv.
...
def add_cors_headers(response):
    """
    Add CORS headers to every response so the dashboard can access the agent.

    Args:
        response (flask.Response): Response object generated by Flask.

    Returns:
        flask.Response: Response with CORS headers applied.
    """
...
def detect_serial_ports():
    """
    Handle CORS preflight OPTIONS requests for any /agent/* endpoint.

    Args:
        path (str): Subpath under /agent/ that the browser is preflighting.
...
# check endpoints (required by dashboard)
@app.route("/agent/ping", methods=["GET"])
def ping():
    """
    Simple check endpoint to confirm the agent is running.
...
def agent_info():
    """
    Return agent connection status for app.py and the dashboard.
...
def detect_ports_endpoint():
    """
    Detect and return available serial ports for the dashboard.
...
def agent_config():
    """
    Receive and save robot configuration data sent from the main server.
...
def agent_connect():
    """
    Upload a data collection script to the robot and execute it.
...
def agent_pull():
    """
    Pull CSV data from the robot and return its contents.
...
def agent_upload():
    """
    Upload a replay script to the robot and optionally execute it.
...
def agent_run():
    """
    Execute the previously uploaded replay.py script on the robot.
...
def not_found(error):
    """
    Handle invalid endpoints requested from the agent.
...
def internal_error(error):
    """
    Handle unexpected internal agent server errors.
