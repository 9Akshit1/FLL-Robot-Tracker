...
async def main():
    for dt, da, db, dc in timeline:
        # Initial delay per step
        if dt > 0:
            await runloop.sleep_ms(dt)

        # Start A and B simultaneously (no await)
        if da != 0:
            motor.run_for_degrees(port.A, da, 1000) # Increased speed

        if db != 0:
            motor.run_for_degrees(port.B, db, 1000)

        # Await the last motor so the step completes before starting the next one
        if dc != 0:
            await motor.run_for_degrees(port.C, dc, 1000)
        else:
            # If motor C isn't moving, add a tiny sleep to keep timing consistent
